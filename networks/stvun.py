...
class EFST(torch.nn.Module):
    def __init__(self, nif=64):
        ...
        self.early_fusion = torch.nn.Conv2d(nif*7, nif, kernel_size=1, padding=0)
    ...
    def forward(self , enc_fs):
        ...
        for f in range(len(enc_fs[0])):
            ...
            ef = self.early_fusion(enc_fs_concat)
            feat_fusion = self.ff(enc_fs_stack, ef)
            
            efst_outs.append(feat_fusion)
